...
授权是一种业务逻辑，描述给定的用户/会话/上下文是否具有执行操作或查看一条数据的权限。例如：
...
我们建议将完全混合的 User 对象传递给业务逻辑层，而非传递不透明的 token 或 API 密钥。这样，我们可以在请求处理管道的不同阶段处理[身份验证](/graphql-js/authentication-and-express-middleware/)和授权的不同问题。
